...
export default class Run extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      test_res_body: null,
      test_res_body_showy: {'message': '请求点啥~~'},
      setting_data_style: '浅色',
      setting_data_collapsed: 3,
      isJSON: true,
      ...this.props.data
    };
  }
  ...
  async initState(data) {
    ...
    let isJSON = this.isJSON(data.test_res_body);
    let json = {};
    if (isJSON) {
      json = JSON.parse(data.test_res_body);
    } else {
      json = data.test_res_body;
    }
    for (let i = 0; i < data.req_query.length; i++) {
      data.req_query[i].value = data.req_query[i].example;
      if (data.req_query[i].value && data.req_query[i].value != '') {
        data.req_query[i].enable = true;
      } else {
        data.req_query[i].enable = false;
      }
    ...
    this.setState(
      {
        ...
        test_valid_msg: null,
        resStatusText: null,
        test_res_body_showy: json,
        isJSON: isJSON
      },
      () => this.props.type === 'inter' && this.initEnvState(data.case_env, data.env)
    );
    let cacheData = this.storageUtil.get('res_body');
    let interfaceId = this.props.interfaceId;
    let isJSON_cacheData = this.isJSON(cacheData.val);
    if (cacheData !== null || cacheData !== '') {
      if (interfaceId == cacheData.interfaceId) {
        this.setState({
          test_res_body_showy: JSON.parse(cacheData.val),
          isJSON: isJSON_cacheData
        });
      }
    } else {
  ...
  initEnvState(case_env, env) {
    ...
    let storageUtil_case_env = this.storageUtil.get('case_env');
    let setting_data_style = this.storageUtil.get('setting_data_style');
    let setting_data_collapsed = this.storageUtil.get('setting_data_collapsed');
    if(setting_data_style){
      this.setState({
        setting_data_style: setting_data_style.val
      });
    }
    if(setting_data_collapsed){
      this.setState({
        setting_data_collapsed: setting_data_collapsed.val
      });
    }
    this.setState(
      {
      ...
      () => {
        ...
        if (storageUtil_case_env) {
          // if(storageUtil_case_env.interfaceId == this.props.interfaceId){
          this.setState({
            case_env: storageUtil_case_env.val
          });
          // }
        }
      }
  ...
  reqRealInterface = async () => {
    ...
    let isJSON = this.isJSON(result.body);
    let json = {};
    if (isJSON) {
    ...
    this.storageUtil.put('res_body', this.props.interfaceId, JSON.stringify(json));
    this.storageUtil.put('case_env', this.props.interfaceId, this.state.case_env);
    console.info('扩展程序打开cross-request的 [背景页],才能看到您的请求原生信息哦');
  };
  ...
  onSelectstyle = (v) => {
    this.setState({
      setting_data_style: v
    });
    this.storageUtil.put('setting_data_style', 1, v);
  };
  onSelectcollapsed = (v) => {
    this.setState({
      setting_data_collapsed: v
    });
    this.storageUtil.put('setting_data_collapsed', 1, v);
  };
  ...
  storageUtil = {
     ...
     * @returns {boolean}
     */
    put: function (key, interfaceId, data, time) {
      try {
        if (!localStorage) {
          return false;
        }
        ...
        var cacheVal = {interfaceId: interfaceId, val: data, exp: cacheExpireDate};
  ...
  copyUrl = url => {
    copy(JSON.stringify(url));
    message.success('已经成功复制到剪切板');
  };
  copyUrl_1 = (env_url, path) => {
    let url = '';
    for (let i = 0; i < this.state.env.length; i++) {
      if (this.state.env[i].name == env_url) {
        url = this.state.env[i].domain;
      }
    }
    copy(url + path);
    message.success(url + path + ' copied');
  };
  copyUrl_2 = (path) => {
    copy(path);
    ...
    return (
      <div className="interface-test postman">
        ...
        <div className="url">
          ...
          <Icon
            type="copy"
            className="url-copy-icon1"
            onClick={() => this.copyUrl_1(case_env, path)}
          />
          <Icon
            type="file-ppt"
            className="url-copy-icon2"
            onClick={() => this.copyUrl_2(path)}
          />
          <Tooltip
            placement="bottom"
            title={(() => {
          ...
          <Tooltip
            placement="bottom"
            title={() => {
              return this.props.type === 'inter' ? '我们推荐您至少保存一个到您自己的测试用例中,因为测试用例的数据会被保存,方便您或者您的队友查看呐' : '更新该用例';
            }}
          >
        ...
        <Collapse defaultActiveKey={['0', '1', '3']} bordered={true}>
          <Panel
        ...
        <Tabs size="small" defaultActiveKey="res1" className="response-tab">
          <Tabs.TabPane tab="showy-response-body" key="res1">
            <Spin spinning={this.state.loading} style={{position: 'relative'}}>
              <div
              ...
              <div  className={
                this.state.setting_data_style == '浅色'
                  ? 'tint ReactJson_content'
                  : 'dark ReactJson_content'
              }>
                <ReactJson
                  theme={this.state.setting_data_style == '浅色'?'rjv-default':'codeschool'}
                  src={this.state.test_res_body_showy}
                  collapsed={this.state.setting_data_collapsed}
                  name={false}
                  indentWidth={2}
                  displayDataTypes={false}
                  collapseStringsAfterLength={130}
                />
              </div>
              <div style={{display: this.state.isJSON ? 'none' : ''}}>
                <AceEditor
                  callback={editor => {
              ...
              <div style={{display: this.isObject(this.state.test_res_body_showy) ? 'none' : ''}}>
                {this.state.test_res_body}
              </div>
            </Spin>
          </Tabs.TabPane>
          <Tabs.TabPane tab="response" key="res">
            <Spin spinning={this.state.loading}>
              ...
              <div>
                <a rel="noopener noreferrer" target="_blank" href="https://juejin.im/post/5c888a3e5188257dee0322af">YApi
                  新版如何查看 http 请求数据</a>
              </div>
          ...
          <Tabs.TabPane tab="setting" key="setting">
            <Form >
              <Form.Item labelCol={{span: 5}} wrapperCol={{span: 12}} label="主题">
                <Select defaultValue={this.state.setting_data_style} style={{width: 200}}
                        onSelect={this.onSelectstyle}>
                  <Option value="浅色">浅色</Option>
                  <Option value="深色">深色</Option>
                </Select>
              </Form.Item>
              <Form.Item labelCol={{span: 5}} wrapperCol={{span: 12}} label="折叠位置">
                <Select defaultValue={this.state.setting_data_collapsed} style={{width: 200}}
                        onSelect={this.onSelectcollapsed}>
